...
def billing(request):
    ...
    if open_bill:
        if len(open_bill) > 1:
            # messages.error(request, '{} open bills exist, clear first.'.format(len(open_bill)))
            messages.error(request, 'มี {} บิล ยังค้างอยู่ ปิดให้เรียบร้อยก่อน'.format(len(open_bill)))
        else:
            # messages.error(request, '{} open bill exists, clear first.'.format(len(open_bill)))
            messages.error(request, 'มี {} บิล ยังค้างอยู่ ปิดให้เรียบร้อยก่อน'.format(len(open_bill)))
        return HttpResponseRedirect(reverse_lazy('admin_page'))
    else:
        # --------------------------------------------------------------------------------
...
def pay_bill(request, bref):
    ...
    if request.method == 'POST':
        ...
        if pay_form.is_valid():
            ...
            if (cd['payment_amount'] - tenant_bill.bill_total) >= 0.5:  # compensate for round_up
                # messages.error(request, 'Invalid input for Room: {}, try again..'.format(rmn))
    ...
    if request.method == 'POST':
        # messages.success(request, 'Room {}, payment completed.'.format(rmn))
...
def delete_user(request, rmn):
    ...
    try:
        ...
        if b_status == 'close':
            ...
            messages.success(request, 'ผู้เช่า {} ห้อง {} ได้ถูกลบออกจากระบบแล้ว !!!'.format(user, rmn))

        else:
            # messages.error(request, 'Bill is Open, cannot be deleted !!!')
            messages.error(request, 'บิลยังค้างอยู่ ไม่สามารถลบได้ !!!')

    except:
        user.delete()
        # messages.error(request, 'No Bill but User has been deleted')
        messages.success(request, 'ผู้เช่า {} ห้อง {} ได้ถูกลบออกจากระบบแล้ว !!!'.format(user, rmn))
...
def maintenance_charge(request):
    ...
    if request.method == 'POST':
        ...
        ms.save()
        # messages.success(request, 'Maintenance Service has been charged to room: {} for {}'.format(rn, jc))
